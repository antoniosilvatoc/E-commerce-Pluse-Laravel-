@forelse($products as $key => $product)
   <tr>
      ...
      <td><a class="click_me bolder" data-path="/product/show" title="Mostrar Produto"
             href="{{ route('product.show',$product->product_id) }}">{{ $product->product_name }}</a>
      </td>
      <td>{{ $product->sku }}</td>
      <td>{{ $product->buy_price }}</td>
      <td>{{ $product->is_off == 1 ? $product->sale_price - $product->off_price : $product->sale_price }}</td>
